...
namespace WinFormsApp1
{
    ...
    {
        public virtual void push_back(ref Shape[] arr, ref int current_size, Shape obj) { }
    }
    ...
    public class Container
    {
        ...
        public void checkCursorAll(int mouseX, int mouseY, bool checkBox1State, bool checkBox2State, int chooseClassIndex, int pictureBox1_Left, int pictureBox1_Right, int pictureBox1_Top, int pictureBox1_Bottom)
        {
            ...
            {
                unselectall();
                if (mouseX >= pictureBox1_Left+30 && mouseX <= pictureBox1_Right - 30 && mouseY >= pictureBox1_Top+30 && mouseY <= pictureBox1_Bottom - 30)
                {
        ...
        public void changeSizeAll(bool setting, int pictureBoxLeft, int pictureBoxRight, int pictureBoxTop, int pictureBoxBottom)
        {
            foreach(Shape p in container)
            {
               if (p!= null && p.selected())
                   p.change_size_ratio(setting, pictureBoxLeft,pictureBoxRight, pictureBoxTop,pictureBoxBottom);
            }
        }

        public void moveShapes_all(int dx, int dy, int pictBoxLeft, int pictBoxRight, int pictBoxTop, int pictBoxBottom)
        {
            foreach (Shape shape in container)
            {
                if (shape != null)
